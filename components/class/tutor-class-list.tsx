        ...
        <Card key={application.id} className="w-full">
          <CardHeader className="pb-2">
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="text-xl">{application.classes.subject}</CardTitle>
                <CardDescription>
                  {getLevelText(application.classes.level)} • {application.classes.province},{" "}
                  {application.classes.district}
                </CardDescription>
              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p>
                  {Array.isArray(application.classes.profiles)
                    ? application.classes.profiles[0]?.full_name
                    : application.classes.profiles?.full_name || "Không xác định"}
                  {application.classes.profiles &&
                    (Array.isArray(application.classes.profiles)
                      ? application.classes.profiles[0]?.gender &&
                        ` (${getGenderText(application.classes.profiles[0]?.gender)})`
                      : application.classes.profiles.gender &&
                        ` (${getGenderText(application.classes.profiles.gender)})`)}
                </p>
              </div>
              <div>
                <h4 className="font-medium text-sm text-muted-foreground mb-1">Địa chỉ</h4>
                <p>{application.classes.address}</p>
              </div>
              <div>
                <h4 className="font-medium text-sm text-muted-foreground mb-1">Lịch học</h4>
                <p>{application.classes.schedule}</p>
              </div>
              <div>
